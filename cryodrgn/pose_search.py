...
def rot_2d_batch(angle, device=None):
    B = len(angle)
...
class PoseSearch_helical_refine:
    ...
    def __init__(
        ...
        self.list_index = np.repeat(self.list_index,n_in_plane_rot)*n_in_plane_rot+np.tile(np.arange(n_in_plane_rot),len(self.list_index))
        print(len(self.list_index), self.list_index)
